...
function calcularPrecioAppDesdeBase(precioBase) {
  return redondearPrecio(Number(precioBase || 0) * 1.06)
}

function calcularPrecioBaseDesdeApp(precioApp) {
  return redondearPrecio(Number(precioApp || 0) / 1.06)
}
            ...
            ? producto.variantes.map((variante, varianteIndex) => {
                const precioBase =
                  variante?.precioBase !== undefined && variante?.precioBase !== null
                    ? redondearPrecio(variante.precioBase)
                    : redondearPrecio(variante?.precio || 0)

                const precio =
                  variante?.precioBase !== undefined && variante?.precioBase !== null
                    ? redondearPrecio(variante?.precio ?? calcularPrecioAppDesdeBase(precioBase))
                    : calcularPrecioAppDesdeBase(precioBase)
            ...
            ? producto.extras.map((extra, extraIndex) => {
                const precioBase =
                  extra?.precioBase !== undefined && extra?.precioBase !== null
                    ? redondearPrecio(extra.precioBase)
                    : redondearPrecio(extra?.precio || 0)

                const precio =
                  extra?.precioBase !== undefined && extra?.precioBase !== null
                    ? redondearPrecio(extra?.precio ?? calcularPrecioAppDesdeBase(precioBase))
                    : calcularPrecioAppDesdeBase(precioBase)
...
const SUPABASE_ORDERS_KEY = "orders"
const SUPABASE_DRIVERS_KEY = "drivers"
...
async function cargarColeccionDesdeNube(key) {
  try {
    const { data, error } = await supabase
      .from(SUPABASE_TABLE)
      .select("data")
    ...
    console.error(`Error cargando colección ${key}:`, error)
    return []
  }
}

async function guardarColeccionEnNube(key, items) {
  const { error } = await supabase
    .from(SUPABASE_TABLE)
    .upsert({
      key,
...
function App() {
  ...
  useEffect(() => {
    ...
    const inicializarRestaurantes = async () => {
      try {
        ...
        const [cargados, pedidosCargados, repartidoresCargados] = await Promise.all([
          cargarRestaurantesDesdeNube(),
          cargarColeccionDesdeNube(SUPABASE_ORDERS_KEY),
          cargarColeccionDesdeNube(SUPABASE_DRIVERS_KEY)
        ])

        if (activo) {
          setRestaurantes(cargados)
          setPedidos(Array.isArray(pedidosCargados) ? pedidosCargados : [])
  ...
  const agregarConfiguradoAlCarrito = () => {
    ...
    const precioUnitarioBase =
      Number(varianteSeleccionada.precioBase ?? calcularPrecioBaseDesdeApp(varianteSeleccionada.precio)) +
      extras.reduce(
        (acc, extra) =>
          acc + Number(extra.precioBase ?? calcularPrecioBaseDesdeApp(extra.precio)),
        0
      )
  ...
  const guardarPedidoEnSistema = async () => {
    ...
    try {
      const subtotalBase = carrito.items.reduce(
        (acc, item) =>
          acc + Number(item.precioBase ?? calcularPrecioBaseDesdeApp(item.precio)) * item.quantity,
        0
      )

      const subtotalApp = carrito.items.reduce(
        (acc, item) => acc + Number(item.precio || 0) * item.quantity,
        0
      )

      const platformCommission10 = redondearPrecio(subtotalBase * 0.1)
      const platformMarkup6 = redondearPrecio(subtotalApp - subtotalBase)
      const deliveryFeeCharged = datosCliente.tipoEntrega === "domicilio" ? ENVIO_FIJO : 0
      const driverPayment = datosCliente.tipoEntrega === "domicilio" ? 30 : 0
      const platformDeliveryProfit = datosCliente.tipoEntrega === "domicilio" ? 5 : 0
      const restaurantPayout = redondearPrecio(subtotalBase - platformCommission10)
      const platformProfitTotal = redondearPrecio(
        platformCommission10 + platformMarkup6 + platformDeliveryProfit
      )

      const nuevoPedido = {
        id: `ord-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
        folio: `TE-${String(Date.now()).slice(-6)}`,
        createdAt: new Date().toISOString(),
        status: "nuevo",
        restaurantId: carrito.restauranteId,
        restauranteNombre: carrito.restauranteNombre,
        driverId: null,
        driverNombre: "",
        customer: {
          nombre: datosCliente.nombre.trim(),
          telefono: datosCliente.telefono.trim(),
          direccion: datosCliente.tipoEntrega === "domicilio" ? datosCliente.direccion.trim() : "",
          notas: datosCliente.notas.trim(),
          tipoEntrega: datosCliente.tipoEntrega,
          metodoPago: datosCliente.metodoPago,
          efectivoCon: datosCliente.metodoPago === "efectivo" ? datosCliente.efectivoCon.trim() : ""
        },
        items: carrito.items.map((item) => ({
          ...
          precioBase: redondearPrecio(item.precioBase ?? calcularPrecioBaseDesdeApp(item.precio)),
          precioApp: redondearPrecio(item.precio || 0),
          totalBase: redondearPrecio(
            Number(item.precioBase ?? calcularPrecioBaseDesdeApp(item.precio)) * item.quantity
          ),
          totalApp: redondearPrecio(Number(item.precio || 0) * item.quantity)
        })),
        subtotalBase: redondearPrecio(subtotalBase),
        subtotalApp: redondearPrecio(subtotalApp),
        platformCommission10,
        platformMarkup6,
        deliveryFeeCharged,
        driverPayment,
        ...
        restaurantPayout,
        platformProfitTotal,
        totalCustomer: redondearPrecio(subtotalApp + deliveryFeeCharged)
      }

      const pedidosActualizados = [nuevoPedido, ...pedidos]
      const guardados = await guardarColeccionEnNube(SUPABASE_ORDERS_KEY, pedidosActualizados)
      setPedidos(guardados)

      alert(`Pedido ${nuevoPedido.folio} recibido correctamente.`)
  ...
  const guardarPedidosDesdeAdmin = async (nuevosPedidos) => {
    const guardados = await guardarColeccionEnNube(SUPABASE_ORDERS_KEY, nuevosPedidos)
    setPedidos(guardados)
  }

  const guardarRepartidoresDesdeAdmin = async (nuevosRepartidores) => {
    const guardados = await guardarColeccionEnNube(SUPABASE_DRIVERS_KEY, nuevosRepartidores)
    setRepartidores(guardados)
  }
...
>
                ...
                <button
                  ...
                  onClick={guardarPedidoEnSistema}
                >
